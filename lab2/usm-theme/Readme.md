...
Для любой темы WordPress обязательными являются:

- style.css
- index.php

Файл style.css содержит информацию о теме и стили, а index.php служит основным шаблоном вывода.
...
Это позволяет не дублировать один и тот же код в разных шаблонах.

### 3. Чем отличаются index.php, single.php и page.php?

- index.php — основной универсальный шаблон, используется по умолчанию
- single.php — шаблон для отображения одной записи
- page.php — шаблон для отображения одной страницы

То есть single.php применяется для постов, а page.php — для статических страниц.
...
- подключают стили и скрипты
- регистрируют меню
- добавляют поддержку миниатюр
- настраивают дополнительные возможности темы
